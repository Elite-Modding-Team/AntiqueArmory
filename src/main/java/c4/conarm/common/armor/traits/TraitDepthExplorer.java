...
public class TraitDepthExplorer extends AbstractArmorTrait {

    private static final float TOUGH_PER_LEVEL = 2.0F;
    private static final int[] ARMOR_VALUES = new int[]{2, 3, 4, 2};

    public TraitDepthExplorer() {
        super("depth_explorer", TextFormatting.YELLOW);
    }

    @Override
    public ArmorModifications getModifications(EntityPlayer player, ArmorModifications mods, ItemStack armor, DamageSource source, double damage, int slot) {
        int y = player.getPosition().getY();
        y = 72 - y; // Actual sea level is 64, we chose 72 because we're nice
